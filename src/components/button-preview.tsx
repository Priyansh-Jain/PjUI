...
export function TailwindcssButtons() {
  ...
  return (
    <div className="pb-12 px-4 w-full mt-10">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full  max-w-7xl mx-auto gap-10">
        {buttons.map((button, idx) => (
          <div
            key={idx}
            className={
              "h-60 w-full bg-white rounded-xl border border-neutral-100 dark:bg-black dark:border-white/[0.2] hover:border-neutral-200 group/btn overflow-hidden relative flex items-center justify-center z-0"
            }
          >
            <div className="relative z-40">
              <div onClick={() => copy(button)}>{button.component}</div>
            </div>
          </div>
        ))}
      </div>
...
export const buttons = [
  {
    name: "Rounded Pulse",
    description: "Rounde Pulse button for your website",
    component: (
      <button className="relative inline-block px-10 py-3 text-gray-700 uppercase bg-white rounded-full font-bold transition-all duration-200 hover:-translate-y-1 hover:shadow-lg active:translate-y-0.5 active:shadow-md btn-animated">
        Hover me
      </button>
    ),
  },
  {
    ...
    component: (
      <button className="px-4 py-2 rounded-md border border-black bg-white text-black text-sm hover:shadow-[40px_40px_0px_0px_rgba(255, 255, 255)] transition duration-200">
        Sketch
      </button>
    ),
  },
